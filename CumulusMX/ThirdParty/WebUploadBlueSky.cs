...
{
	internal class WebUploadBlueSky(Cumulus cumulus, string name) : WebUploadServiceBase(cumulus, name)
	{
		private string authToken;
		private string did;

		//CancellationToken token, string userUrl = null, string userUrlPreamble = "Website: ", string baseUrl = "https://bsky.social"
		...
		private static List<Facet> detectLinkFacets(string content, out string modifiedContent)
		{
			...
			while ((match = regex.Match(content[pos..])).Success)
			{
				...
				{ 
					...
					int start = content.IndexOf(url, match.Index);

					facet.index.byteStart = start + 1;
					facet.index.byteEnd = start + label.Length + 1;
		...
		private static List<Facet> detectTagFacets(string content)
		{
			...
			var regex = new Regex(@"(?:^|\s)(#[^\d\s]\S*)(?=\s|$)", RegexOptions.Compiled);
			...
			{ 
				...
				{ 
					...
					int start = content.IndexOf(tag, match.Index);

					facet.index.byteStart = start;
					facet.index.byteEnd = start + tag.Length + 1;
