...
namespace CumulusMX.ThirdParty
{
	internal class WebUploadWindGuru(Cumulus cumulus, string name) : WebUploadServiceBase(cumulus, name)
	{
		...
		internal override string GetURL(out string pwstring, DateTime timestamp)
		{
			...
			double maxwind = 0;
			double minwind = 999;
			var cutoff = (DateTime.UtcNow - TimeSpan.FromMinutes(cumulus.WindGuru.Interval)).ToUnixTime();
			...
			lock (station.recentwindLock)
			{
				...
				for (var node = station.WindRecent.Last; node != null; node = node.Previous)
				{
					...
					numvalues++;
					totalwind += node.Value.Gust;

					if (node.Value.Gust > maxwind)
					{
						maxwind = node.Value.Gust;
					}

					if (node.Value.Gust < minwind)
					{
						minwind = node.Value.Gust;
					}
				}
			}
			// average the values
