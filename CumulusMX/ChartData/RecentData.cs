...
namespace CumulusMX
{
	public partial class Cumulus
	{
		public string GetSolarGraphData(bool incremental, bool local, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.SOLAR].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
			}

			var data = Stations[0].RecentDataDb.Query<RecentData>("select * from RecentData where Timestamp > ? order by Timestamp", dateFrom.ToUnixTime());
			...
			for (var i = 0; i < data.Count; i++)
			{
				...
				{
					sbUv.Append($"[{jsTime},{(data[i].UV.HasValue ? data[i].UV.Value.ToString(UVFormat, InvC) : "null")}],");
				}
		...
		public string GetRainGraphData(bool incremental, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.RAIN].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
			...
			for (var i = 0; i < data.Count; i++)
			{
				...
				sbRain.Append($"[{jsTime},{data[i].RainToday.ToString(RainFormat, InvC)}],");

				sbRate.Append($"[{jsTime},{data[i].RainRate.ToString(RainFormat, InvC)}],");
			}
		...
		public string GetHumGraphData(bool incremental, bool local, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.HUM].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
		...
		public string GetWindDirGraphData(bool incremental, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.WINDDIR].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
		...
		public string GetWindGraphData(bool incremental, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.WIND].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
			...
			for (var i = 0; i < data.Count; i++)
			{
				...
				sb.Append($"[{jsTime},{data[i].WindGust.ToString(WindFormat, InvC)}],");

				sbSpd.Append($"[{jsTime},{data[i].WindSpeed.ToString(WindAvgFormat, InvC)}],");
			}
		...
		public string GetPressGraphData(bool incremental, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.PRESS].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
			...
			for (var i = 0; i < data.Count; i++)
			{
				...
				sb.Append($"[{jsTime},{data[i].Pressure.ToString(PressFormat, InvC)}],");
			}
		...
		public string GetTempGraphData(bool incremental, bool local, DateTime? start = null)
		{
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.TEMP].LastDataTime;
			}
			else
			{
				dateFrom = DateTime.Now.AddHours(-GraphHours);
			...
			for (var i = 0; i < data.Count; i++)
			{
				...
				{
					var val = data[i].IndoorTemp.ToFixed(TempFormat, "null");
					sbIn.Append($"[{jsTime},{val}],");
				}

				if (GraphOptions.Visible.DewPoint.IsVisible(local))
					sbDew.Append($"[{jsTime},{data[i].DewPoint.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.AppTemp.IsVisible(local))
					sbApp.Append($"[{jsTime},{data[i].AppTemp.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.FeelsLike.IsVisible(local))
					sbFeel.Append($"[{jsTime},{data[i].FeelsLike.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.WindChill.IsVisible(local))
					sbChill.Append($"[{jsTime},{data[i].WindChill.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.HeatIndex.IsVisible(local))
					sbHeat.Append($"[{jsTime},{data[i].HeatIndex.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.Temp.IsVisible(local))
					sbTemp.Append($"[{jsTime},{data[i].OutsideTemp.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.Humidex.IsVisible(local))
					sbHumidex.Append($"[{jsTime},{data[i].Humidex.ToFixed(TempFormat)}],");

				if (GraphOptions.Visible.BGT.IsVisible(local))
				{
					sbBgt.Append($"[{jsTime},{data[i].BGT.ToFixed(TempFormat, "null")}],");
					sbWbgt.Append($"[{jsTime},{data[i].WBGT.ToFixed(TempFormat, "null")}],");
				}
			}
		...
		public string GetAqGraphData(bool incremental, DateTime? start = null, DateTime? end = null)
		{
				...
				&& StationOptions.PrimaryAqSensor != (int) PrimaryAqSensor.AirLinkIndoor)
			{
				...
				if (incremental)
				{
					dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.AIRQUAL].LastDataTime;
				}
		...
		public string GetExtraTempGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.ExtraTemp.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.ExtraTemp.Vals.Length; i++)
			{
				if (GraphOptions.Visible.ExtraTemp.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.ExtraTempCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.EXTRATEMP].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				dateFrom = dateFrom.AddHours(-GetHourInc(dateFrom));
				dateTo = dateTo.AddHours(-GetHourInc(dateTo));
			}
			else
			{
			...
			var logFile = GetExtraLogFileName(fileDate);
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
								...
								if (entryTs > tsFrom)
								{
									...
									for (var i = 0; i < GraphOptions.Visible.ExtraTemp.Vals.Length; i++)
									{
										if (GraphOptions.Visible.ExtraTemp.ValVisible(i, local))
										{
											...
											{
												val = double.TryParse(st[fields[i]], InvC, out temp) ? temp.ToFixed(TempFormat) : "null";
											}
							...
							{
								errorCount++;
								LogErrorMessage($"GetExtraTempGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetExtraTempGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetExtraTempGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}

				if (!finished)
				{
					if (entryTs >= tsTo || MeteoDate(fileDate) > MeteoDate(dateTo))
					{
						finished = true;
					}
					else
					{
						...
						logFile = GetExtraLogFileName(fileDate);
					}
				}
			} while (!finished);
		...
		public string GetExtraDewPointGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.ExtraDewPoint.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.ExtraDewPoint.Vals.Length; i++)
			{
				if (GraphOptions.Visible.ExtraDewPoint.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.ExtraDPCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.EXTRADEW].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetExtraDewpointGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetExtraDewpointGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetExtraDewpointGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetExtraHumGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.ExtraHum.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.ExtraHum.Vals.Length; i++)
			{
				if (GraphOptions.Visible.ExtraHum.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.ExtraHumCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.EXTRAHUM].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
			...
			// get the log file name to start
			var logFile = GetExtraLogFileName(dateFrom);
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetExtraHumGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetExtraHumGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetExtraHumGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetSoilTempGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.SoilTemp.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.SoilTemp.Vals.Length; i++)
			{
				if (GraphOptions.Visible.SoilTemp.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.SoilTempCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.SOILTEMP].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetSoilTempGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetSoilTempGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetSoilTempGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetSoilMoistGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.SoilMoist.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.SoilMoist.Vals.Length; i++)
			{
				if (GraphOptions.Visible.SoilMoist.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.SoilMoistureCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.SOILMOIST].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetSoilMoistGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetSoilMoistGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetSoilMoistGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetSoilEcGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.SoilEc.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.SoilEc.Vals.Length; i++)
			{
				if (GraphOptions.Visible.SoilEc.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.SoilEcCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.SOILEC].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetSoilEcGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetSoilEcGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetSoilEcGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetLaserDepthGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.LaserDepth.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.LaserDepth.Vals.Length; i++)
			{
				if (GraphOptions.Visible.LaserDepth.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.LaserCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.LASERDEPTH].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
								...
								if (entryTs > tsFrom)
								{
									...
									for (var i = 0; i < GraphOptions.Visible.LaserDepth.Vals.Length; i++)
									{
										if (GraphOptions.Visible.LaserDepth.ValVisible(i, local))
										{
											...
											if (fields[i] < st.Count)
											{
												val = double.TryParse(st[fields[i]], InvC, out temp) ? temp.ToString(LaserFormat, InvC) : "null";
											}
											sbExt[i].Append($"[{jsTime},{val}],");
										}
									}
							...
							{
								errorCount++;
								LogErrorMessage($"GetLaserDepthGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetLaserDepthGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetLaserDepthGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetSnow24hGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.SNOW24H].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
			...
			do
			{
				if (File.Exists(logFile))
				{
						...
						foreach (var line in lines)
						{
							try
							{
								...
								if (entryTs > tsFrom)
								{
									...
									if (field < st.Count)
									{
										val = double.TryParse(st[field], InvC, out temp) ? temp.ToString(SnowFormat, InvC) : "null";
									}
							...
							{
								errorCount++;
								LogErrorMessage($"GetSnow24hGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetSnow24hGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetSnow24hGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetLeafWetnessGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.LeafWetness.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.LeafWetness.Vals.Length; i++)
			{
				if (GraphOptions.Visible.LeafWetness.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.LeafWetnessCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.LEAFWET].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetLeafWetnessGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetLeafWetnessGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetLeafWetnessGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetUserTempGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			var sbExt = new StringBuilder[GraphOptions.Visible.UserTemp.Vals.Length];

			for (var i = 0; i < GraphOptions.Visible.UserTemp.Vals.Length; i++)
			{
				if (GraphOptions.Visible.UserTemp.ValVisible(i, local))
					sbExt[i] = new StringBuilder($"\"{Trans.UserTempCaptions[i]}\":[");
			}
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.USERTEMP].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							...
							{
								errorCount++;
								LogErrorMessage($"GetUserTempGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetUserTempGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetUserTempGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
		...
		public string GetCo2SensorGraphData(bool incremental, bool local, DateTime? start = null, DateTime? end = null)
		{
			...
			*/
			...
			if (incremental)
			{
				dateFrom = start ?? GraphDataFiles[(int) GraphFileIdx.CO2].LastDataTime;
				dateTo = DateTime.Now;
			}
			else if (start.HasValue && end.HasValue)
			{
			...
			do
			{
				if (File.Exists(logFile))
				{
					...
					try
					{
						...
						foreach (var line in lines)
						{
							try
							{
								...
								var rec = new LogFiles.ExtraLogFileRec(line);
								...
								if (entryTs > tsFrom)
								{
									...
									if (GraphOptions.Visible.CO2Sensor.Temp.IsVisible(local))
										sbTemp.Append($"[{jsTime},{rec.CO2_temperature.ToFixed(TempFormat, "null")}],");
							...
							{
								errorCount++;
								LogErrorMessage($"GetCo2SensorGraphData: Error at line {linenum} of {logFile}. Error - {ex.Message}");
								if (errorCount > 10)
								{
									LogMessage($"GetCo2SensorGraphData: More than 10 errors in the file {logFile}, aborting processing");
									finished = true;
									break;
								}
							}
					...
					{
						LogErrorMessage($"GetCo2SensorGraphData: Error reading {logFile}. Error - {ex.Message}");
					}
				}
