...
namespace CumulusMX
{
    public class Cumulus
    {
        ...
        private const int DefaultRedMeteoInterval = 10;
        ...
        public bool SynchronisedRedMeteoUpdate; // redmeteo.cl
        ...
        private List<string> RedMeteoList = new List<string>();  //redmeteo.cl
        ...
        // REDMETEO.CL SETTINGS (redmeteo.cl)
        public string RedMeteoID = " ";
        public bool RedMeteoEnabled = false;
        public int RedMeteoInterval = 2;
        ...
        private static HttpClientHandler RedmeteohttpHandler = new HttpClientHandler();
        private HttpClient RedmeteohttpClient = new HttpClient(RedmeteohttpHandler);
        private bool UpdatingRedMeteo = false;
		...
		*/
        ...
        public Cumulus(int HTTPport, int WSport)
        {
			...
			*/
            ...
            LogMessage("Cumulus MX v." + Version + " build " + Build + " Redmeteo.cl edition");
            Console.WriteLine("Cumulus MX v." + Version + " build " + Build + "Redmeteo.cl edition");
            ...
            Console.WriteLine("Corriendo Cumulus en: " + httpServer.Listener.Prefixes.First());
            Console.WriteLine("(Reemplace * con la direccion IP de su maquina, o localhost)");
            ...
            RedMeteoTimer.Elapsed += RedMeteoTimerTick;
...
*/
        ...
        private void RedMeteoTimerTick(object sender, ElapsedEventArgs e)
        {
            if (!String.IsNullOrEmpty(RedMeteoID) && (RedMeteoID != " "))
                UpdateRedMeteo(DateTime.Now);
        }
        ...
        internal async void UpdateRedMeteo(DateTime timestamp)
        {
            // Actualización automática a Redmeteo.cl
            if (!UpdatingRedMeteo)
            {
                UpdatingRedMeteo = true;

                string apistring;

                string URL = station.GetRedMeteoURL(out apistring, timestamp);
                string LogURL = URL.Replace(apistring, "<redmeteo-id>");

                LogDebugMessage("Redmeteo.cl URL: " + LogURL);

                try
                {
                    HttpResponseMessage response = await RedmeteohttpClient.GetAsync(URL);
                    var responseBodyAsText = await response.Content.ReadAsStringAsync();
                    LogMessage("Redmeteo.cl Response: " + response.StatusCode + ": " + responseBodyAsText);
                }
                catch (Exception ex)
                {
                    LogMessage("Redmeteo.cl update: " + ex.Message);
                }
                finally
                {
                    UpdatingRedMeteo = false;
                }
            }
        }
        ...
        private void ReadIniFile()
        {
            ...
            // redmeteo.cl inicializacion de variables
            RedMeteoID = ini.GetValue("Redmeteo", "redmeteoid", "");
            RedMeteoEnabled = ini.GetValue("Redmeteo", "enabled", false);
            RedMeteoInterval = ini.GetValue("Redmeteo", "interval", DefaultRedMeteoInterval);
            if (RedMeteoInterval < 2) { RedMeteoInterval = 2; }

            SynchronisedRedMeteoUpdate = (60 % RedMeteoInterval == 0);
        ...
        internal void WriteIniFile()
        {
            ...
            ini.SetValue("Redmeteo", "redmeteoid", RedMeteoID); //redmeteo.cl
            ini.SetValue("Redmeteo", "enabled", RedMeteoEnabled);
            ini.SetValue("Redmeteo", "interval", RedMeteoInterval);
        ...
        public Timer RedMeteoTimer = new Timer();
        ...
        public void StartTimers()
        {
            ...
            RedMeteoTimer.Interval = RedMeteoInterval * 60 * 1000;  // REDMETEO
        ...
        /// <summary>
        /// Process the list of Windy updates created at startup from logger entries
        /// </summary>
        ...
        ///// <summary>
        ///// Process the list of redmeteo.cl updates created at startup from logger entries
        ///// </summary>

        //private async void RedMeteoCatchup()
        //{
        //    UpdatingRedMeteo = true;
        //    for (int i = 0; i < RedMeteoList.Count; i++)
        //    {
        //        LogMessage("Uploading Redmeteo.cl archive #" + (i + 1));
        //        try
        //        {
        //            HttpResponseMessage response = await RedmeteohttpClient.GetAsync(RedMeteoList[i]);
        //            LogMessage("Redmeteo.cl Response: " + response.StatusCode + ": " + response.ReasonPhrase);
        //        }
        //        catch (Exception ex)
        //        {
        //            LogMessage("Redmeteo.cl update: " + ex.Message);
        ...
        //    LogMessage("End of Redmeteo.cl archive upload");
        //    RedMeteoList.Clear();
        //    RedMeteoTimer.Enabled = RedMeteoEnabled && !SynchronisedRedMeteoUpdate;
        //    UpdatingRedMeteo = false;
        //}
        ...
        private void AddToRedMeteoList(DateTime timestamp)    // redmeteo.cl 
        {
            if (RedMeteoEnabled)
            {
                string apistring;
                ...
                RedMeteoList.Add(URL);

                string LogURL = URL.Replace(apistring, "<redmeteo-id>");

                LogMessage("Creating Redmeteo.cl URL #" + WindyList.Count);
