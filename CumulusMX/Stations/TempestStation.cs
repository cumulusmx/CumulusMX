	...
	internal class TempestStation : WeatherStation
	{
		...
		private void ProcessHistoryData(List<Observation> datalist)
		{
			...
			foreach (var historydata in datalist)
			{
				...
				DoPressure(ConvertUnits.PressMBToUser(MeteoLib.GetSeaLevelPressure(ConvertUnits.AltitudeM(cumulus.Altitude), ConvertUnits.UserPressToHpa(MetData.StationPressure), ConvertUnits.UserTempToC(avgTemp), cumulus.Latitude)), timestamp);
				...
				MetData.TempSamplesToday += historydata.ReportInterval;
