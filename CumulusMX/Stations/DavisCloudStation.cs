...
namespace CumulusMX.Stations
{
	internal partial class DavisCloudStation : WeatherStation
	{
		...
		private async void GetCurrent(object source, ElapsedEventArgs e)
		{
			...
			try
			{
				...
				if (responseCode != 200)
				{
					...
					{
						cumulus.LogErrorMessage($"GetCurrent: WeatherLink API Current HTTP Error: {responseCode}");
					}
					return;
				}
		...
		private void GetHistoricData(BackgroundWorker worker)
		{
			...
			try
			{
				...
				if (responseCode != 200)
				{
					...
					{
						cumulus.LogErrorMessage($"GetHistoricData: WeatherLink API Historic HTTP Error: {responseCode}");
					}
												...
												try
												{
													...
													else
													{
														try
														{
															...
															StartOfStorm = rec.rain_storm_start_at.Value.LocalFromUnixTime();
														}
														catch (Exception ex)
														{
					...
					case 24: // WL console ISS data
						{
							...
							if (cumulus.WllPrimaryTempHum == data.tx_id)
							{
								...
								try
								{
									...
									{
										ts = data.hum_hi_at.LocalFromUnixTime();
									...
									{
										ts = data.hum_lo_at.LocalFromUnixTime();
								...
								try
								{
									...
									else
									{
										...
										{
											ts = data.temp_hi_at.LocalFromUnixTime();
										...
										{
											ts = data.temp_lo_at.LocalFromUnixTime();
								...
								try
								{
									...
									{
										ts = data.dew_point_hi_at.LocalFromUnixTime();
									...
									{
										ts = data.dew_point_lo_at.LocalFromUnixTime();
								...
								if (!cumulus.StationOptions.CalculatedWC)
								{
									...
									try
									{
										...
										{
											ts = data.wind_chill_lo_at.LocalFromUnixTime();
		...
		private void DecodeWllHealth(WlHealthDataType15 data, bool startingup)
		{
			...
			try
			{
				var dat = data.firmware_version.LocalFromUnixTime();
				...
				upt = TimeSpan.FromSeconds(data.link_uptime ?? 0);
		...
		private void GetAvailableStationIds(bool logToConsole = false)
		{
			...
			try
			{
				...
				if (responseCode != 200)
				{
					...
					{
						cumulus.LogErrorMessage($"GetStations: WeatherLink API HTTP Error: {responseCode}");
					}
					return;
				}
