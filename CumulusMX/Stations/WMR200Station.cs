	...
	internal sealed class WMR200Station : WeatherStation
	{
		...
		private void ProcessHistoryDataPacket()
		{
			...
			if (cumulus.StationOptions.CalculateSLP)
			{
				...
				num = MeteoLib.GetSeaLevelPressure(ConvertUnits.AltitudeM(cumulus.Altitude), ConvertUnits.UserPressToMB(StationPressure), ConvertUnits.UserTempToC(avgTemp), cumulus.Latitude);
			}
