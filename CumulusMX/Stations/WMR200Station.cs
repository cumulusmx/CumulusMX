...
namespace CumulusMX.Stations
{
	internal sealed class WMR200Station : WeatherStation
	{
			...
			try
			{
				do
				{
					...
					try
					{
						...
						for (var I = startByte; I < responseLength; I++)
						{
		...
		private void SendHeartBeat()
		{
			...
			if (cumulus.IsOSX)
			{
				heartbeat = [0x01, 0xd0, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00];
			}
			else
			{
				heartbeat = [0x00, 0x01, 0xd0, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00];
			}
		...
		private void SendDA()
		{
			...
			if (cumulus.IsOSX)
			{
				da = [0x01, 0xda, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00];
			}
			else
			{
				da = [0x00, 0x01, 0xda, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00];
			}
		...
		{
			for (var I = 0; I < PacketBufferBound; I++)
			{
				packetBuffer[I] = 0;
			}
		...
		private void RemovePacketFromBuffer()
		{
			...
			var overflow = currentPacketLength - actualpacketlength;
			if (overflow == 0)
			{
			...
			else
			{
				...
				var str = " ";
				for (var I = 0; I < currentPacketLength; I++)
				{
		...
		private void ProcessWindPacket()
		{
			...
			// bearing
			var bearing = (int) ((packetBuffer[7] & 0xF) * 22.5);
			// gust
			var gust = ((packetBuffer[10] & 0xF) * 256 + packetBuffer[9]) / 10.0;
			// average
			var average = (packetBuffer[11] * 16 + packetBuffer[10] / 16) / 10.0;

			DoWind(ConvertUnits.WindMSToUser(gust), bearing, ConvertUnits.WindMSToUser(average), now);
		...
		private void ProcessHistoryDataPacket()
		{
			...
			if (firstArchiveData)
			{
				...
				if (cumulus.LastUpdateTime.AddHours(hourInc).Date == timestamp.AddHours(hourInc).Date)
				{
					...
					var luh = cumulus.LastUpdateTime.Hour;
			...
			else if (!snowhourdone)
			{
				...
				for (var j = 0; j < MetData.Snow24h.Length; j++)
				{
					MetData.Snow24h[j] = MetData.LaserDepth[j].HasValue ? 0 : null;
				}
			...
			// bearing
			var bearing = (int) ((packetBuffer[20] & 0xF) * 22.5);
			// gust
			var gust = ((packetBuffer[23] & 0xF) * 256 + packetBuffer[22]) / 10.0;
			// average
			var average = (packetBuffer[24] * 16 + packetBuffer[23] / 16) / 10.0;
			...
			for (i = 0; i <= sensorcount; i++)
			{
				try
				{
					...
					if (sensornumber == 0)
					{
						...
						DoIndoorTemp(ConvertUnits.TempCToUser(sign * ((packetBuffer[offset + 2] & 0xF) * 256 + packetBuffer[offset + 1])) / 10.0);
					}
					else if (sensornumber == cumulus.WMR200TempChannel)
					{
						...
						DoOutdoorTemp(ConvertUnits.TempCToUser(sign * ((packetBuffer[offset + 2] & 0xF) * 256 + packetBuffer[offset + 1]) / 10.0), timestamp);
						...
						DoOutdoorDewpoint(ConvertUnits.TempCToUser(sign * ((packetBuffer[offset + 5] & 0xF) * 256 + packetBuffer[offset + 4]) / 10.0), timestamp);
					}
					...
					if (sensornumber > 1 && sensornumber < 11)
					{
						...
						DoExtraTemp(ConvertUnits.TempCToUser(sign * ((packetBuffer[offset + 2] & 0xF) * 256 + packetBuffer[offset + 1]) / 10.0), sensornumber);
						...
						DoExtraDP(ConvertUnits.TempCToUser(sign * ((packetBuffer[offset + 5] & 0xF) * 256 + packetBuffer[offset + 4]) / 10.0), sensornumber);
					}
				}
				catch (Exception ex)
				{
			...
			AddRecentDataEntry(timestamp, MetData.WindAverage, MetData.RecentMaxGust, MetData.WindLatest, MetData.Bearing, MetData.AvgBearing, MetData.Temperature, MetData.WindChill, MetData.Dewpoint, MetData.HeatIndex, MetData.Humidity,
							MetData.Pressure, MetData.RainToday, MetData.SolarRad, MetData.UV, RainCounter, MetData.FeelsLike, MetData.Humidex, MetData.ApparentTemperature, MetData.TemperatureIn, MetData.HumidityIn, CurrentSolarMax, MetData.RainRate, -1, -1, MetData.BlackGlobeTemp, MetData.WetBulbGlobeTemp);
		...
		private void ProcessWMR200Packet()
		{
			...
			for (var i = 0; i < packetBuffer[1]; i++)
			{
