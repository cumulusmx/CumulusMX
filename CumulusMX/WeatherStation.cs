...
namespace CumulusMX
{
	internal abstract partial class WeatherStation
	{
		...
		protected WeatherStation(Cumulus cumulus, bool extraStation = false)
		{
			...
			RecentDataDb.CreateTable<CumulusMX.CWindRecent>();
		...
		private void CheckSqliteDatabase(bool giveup)
		{
			...
			if (timestampColumn != null && timestampColumn.ColumnType != "INTEGER")
			{
				RecentDataDb.DropTable<CumulusMX.CWindRecent>();
			}
		}
		...
		public void ReadTodayFile()
		{
			...
			// WBGT
			HiLoToday.HighWbgt = ini.GetValue("WBGT", "High", Cumulus.DefaultHiVal);
		...
		{
			if (BlackGlobeTemp.HasValue && Pressure > 0)
			{
				var wbgt_c = MeteoLib.CalculateWetBulbeGlobeTemp(
					ConvertUnits.UserTempToC(OutdoorTemperature),
					ConvertUnits.UserTempToC(OutdoorDewpoint),
					ConvertUnits.UserPressToHpa(Pressure),
					ConvertUnits.UserTempToC(BlackGlobeTemp.Value)
					);
		...
		public void DoWind(double gustpar, int bearingpar, double speedpar, DateTime timestamp)
		{
			...
			AddNewWindVector(Bearing, timestamp);
		...
		public void AddNewWindVector(int dir, DateTime time)
		{
			lock (recentwindLock)
			{
				WindVec[nextwindvec].X = calibratedgust * Math.Sin(DegToRad(dir));
				WindVec[nextwindvec].Y = calibratedgust * Math.Cos(DegToRad(dir));
...
//					cumulus.LogDebugMessage($"Wind Time:{node.Value.DateTime.ToLocalTime().ToLongTimeString()} Gust:{node.Value.Gust:F1} Speed:{node.Value.Speed:F1}");
		...
		{
			return CompassPoint(GetWindDirAvgFromArray(fromTime));
		}
		...
		public void WriteYesterdayFile(DateTime logdate)
		{
			...
			// BGT
			ini.SetValue("BGT", "High", HiLoYest.HighBgt);
			ini.SetValue("BGT", "HTime", HiLoYest.HighBgtTime);
			// WBGT
			ini.SetValue("WBGT", "High", HiLoYest.HighWbgt);
			ini.SetValue("WBGT", "HTime", HiLoYest.HighWbgtTime);
		...
		public void SaveWindData()
		{
			...
			try
			{
				...
				RecentDataDb.DeleteAll<CumulusMX.CWindRecent>();
...
ORDER BY rd.date ASC;", earliest[0].Date.ToString("yyyy-MM-dd"));
		...
		internal string GetCurrentData()
		{
			...
			var data = new DataStruct(cumulus, OutdoorTemperature, OutdoorHumidity, TempTotalToday / tempsamplestoday, IndoorTemperature, OutdoorDewpoint, WindChill, BlackGlobeTemp, WetBulbGlobeTemp, IndoorHumidity,
