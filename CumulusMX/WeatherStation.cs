...
namespace CumulusMX
{
	internal abstract class WeatherStation
	{
		...
		private const int alltimerecbound = 24;
		...
		*/
...

		{
			...
			//DoAlarm(outsidetemp, alarmhightemp, HighTempAlarmSet, true,
			//  HighTempAlarmState, MainForm.TempLabel, HighTempAlarmSound,
			...
			//DoAlarm(outsidetemp, alarmlowtemp, LowTempAlarmSet, false, LowTempAlarmState,
			//  MainForm.TempLabel, LowTempAlarmSound, LowTempAlarmSoundFile);
		...
		{
			Pressure = sl + cumulus.PressOffset;
			...
			//DoAlarm(slpress , alarmhighpress , HighPressAlarmSet , true, HighPressAlarmState , MainForm.PressLabel , HighPressAlarmSound , HighPressAlarmSoundFile );
			...
			//DoAlarm(slpress , alarmlowpress , LowPressAlarmSet , false, LowPressAlarmState , MainForm.PressLabel , LowPressAlarmSound , LowPressAlarmSoundFile );
		...
		public void DoRain(double total, double rate, DateTime timestamp)
		{
			...
			else
			{
					...
					// Do rain rate
				{
					...
					//DoAlarm(RainRate, alarmhighrainrate, HighRainRateAlarmSet, true,
					//  HighRainRateAlarmState, MainForm.RainRateLabel, HighRainRateAlarmSound,
				...
				if (!FirstChanceRainReset)
				{
					...
					//DoAlarm(raintoday, alarmhighraintoday, HighRainTodayAlarmSet, true,
					//  HighRainTodayAlarmState, MainForm.RainTodayLabel,
					//  HighRainTodayAlarmSound, HighRainTodayAlarmSoundFile);
		...
		public int DavisTotalPacketsMissed = 0;
		public int DavisNumberOfResynchs = 0;
		public int DavisMaxInARow = 0;
		public int DavisNumCRCerrors = 0;
		...
		private void DoDayfile(DateTime timestamp)
		{
			...
			str += highuvtodaytime.ToString("HH:mm");
			...
			try
			{
				...
				using (StreamWriter file = new StreamWriter(fs))
				{
					...
					else
					{
						...
						file.WriteLine(highuvtodaytime.ToString("HH:mm"));
		...
		public void AddGraphDataEntry(DateTime ts, double rain, double raintoday, double rrate, double temp, double dp, double appt, double chill, double heat, double intemp,
			double press, double speed, double gust, int avgdir, int wdir, int hum, int inhum, double solar, double smax, double uv)
		{
			var graphdata = new GraphData(ts, rain, raintoday, rrate, temp, dp, appt, chill, heat, intemp, press, speed, gust, avgdir, wdir, hum, inhum, solar, smax, uv);
							...
							do
							{
								...
								if (entrydate >= datefrom && entrydate <= dateto)
								{
									...
									AddGraphDataEntry(entrydate, raintotal, raintoday, rainrate, outsidetemp, dewpoint, appt, chill, heat, insidetemp, pressure, speed, gust,
										avgbearing, bearing, hum, inhum, solar, solarmax, uv);
								}
							} while (!(sr.EndOfStream || entrydate >= dateto));
						}
					}
        ...
        public string GetRedMeteoURL(out string apistring, DateTime timestamp)
        {
            ...
            // llamada a la URL de redmeteo.cl
            URL = "http://redmeteo.cl/telemetry.php?";

            URL = URL + "&ID=" + cumulus.RedMeteoID;
            URL = URL + "&format=realtime";

            URL = URL + "&date=" + fecha;                                                                       // 1       
            URL = URL + "&time=" + hora;                                                                        // 2

            Data = Data + "&temp=" + TempCstr(OutdoorTemperature) ;                                             // 3
            Data = Data + "&rh=" + OutdoorHumidity;                                                             // 4
            Data = Data + "&dewpoint=" + TempCstr(OutdoorDewpoint);                                             // 5
            Data = Data + "&wsavg=" + WindMPHStr(WindAverage);                                                  // 6
            Data = Data + "&ws=" + WindMPHStr(WindLatest) ;                                                     // 7
            Data = Data + "&wdirangle=" + AvgBearing;                                                           // 8
            Data = Data + "&rainrate=" + RainMMstr(RainRate);                                                   // 9
            Data = Data + "&rain=" + RainMMstr(RainToday);                                                      // 10
            Data = Data + "&baro=" + PressHPAstr(Pressure);                                                     // 11
            Data = Data + "&wdir=" + CompassPoint(Bearing);                                                     // 12
            Data = Data + "&bf=" + Beaufort(WindAverage).ToString("F1");                                        // 13
            Data = Data + "&wsunit=mph";                                                                        // 14
            Data = Data + "&tempunit=C";                                                                        // 15
            Data = Data + "&presunit=hPa";                                                                      // 16
            Data = Data + "&rainunit=mm";                                                                       // 17
            Data = Data + "&windrun=" + WindRunToday.ToString("F1");                                            // 18
            Data = Data + "&prestrend=" + PressHPAstr(presstrendval);                                           // 19
            Data = Data + "&monthrain=" + RainMMstr(RainMonth);                                                 // 20
            Data = Data + "&yearrain=" + RainMMstr(RainYear);                                                   // 21
            Data = Data + "&yesterdayrain=" + RainMMstr(RainYesterday);                                         // 22
            Data = Data + "&insidetemp=" + TempCstr(IndoorTemperature);                                         // 23
            Data = Data + "&insiderh=" + IndoorHumidity;                                                        // 24
            Data = Data + "&windchill=" + TempCstr(WindChill);                                                  // 25
            Data = Data + "&temptrend="+ temptrendval.ToString(cumulus.TempTrendFormat);                        // 26
            Data = Data + "&tempmax="+ TempCstr(HighTempToday);                                                 // 27
            Data = Data + "&htempmax=" + hightemptodaytime.ToString("HH:mm");                                   // 28
            Data = Data + "&tempmin=" + TempCstr(LowTempToday);                                                 // 29
            Data = Data + "&htempmin=" + lowtemptodaytime.ToString("HH:mm");                                    // 30
            Data = Data + "&wsavgmax=" + WindMPHStr(highwindtoday);                                             // 31
            Data = Data + "&hwsavgmax=" + highwindtodaytime.ToString("HH:mm");                                  // 32
            Data = Data + "&wsgustmax=" + WindMPHStr(highgusttoday);                                            // 33
            Data = Data + "&hwsgustmax=" + highgusttodaytime.ToString("HH:mm");                                 // 34
            Data = Data + "&presmax=" + PressHPAstr(highpresstoday);                                            // 35
            Data = Data + "&hpresmax=" + highpresstodaytime.ToString("HH:mm");                                  // 36
            Data = Data + "&presmin=" + PressHPAstr(lowpresstoday);                                             // 37
            ...
            Data = Data + "&tenmingust=" + WindMPHStr(RecentMaxGust);                                           // 41
            Data = Data + "&heatindex=" + HeatIndex.ToString(cumulus.TempFormat);                               // 42
            Data = Data + "&humidex=" + Humidex.ToString(cumulus.TempFormat);                                   // 43
            Data = Data + "&uv=" + UV.ToString(cumulus.UVFormat);                                               // 44
            Data = Data + "&et=" + ET.ToString(cumulus.ETFormat);                                               // 45
            Data = Data + "&sw=" + Convert.ToInt32(SolarRad).ToString();                                        // 46
            Data = Data + "&tenmindir=" + AvgBearing.ToString();                                                // 47
            Data = Data + "&rainlasthr=" + RainMMstr(RainLastHour);                                             // 48
            Data = Data + "&zambretti=" + Forecastnumber.ToString();                                            // 49
            Data = Data + "&isday=" + (cumulus.IsDaylight() ? "1 " : "0 ");                                     // 50
            Data = Data + "&scl=" + (SensorContactLost ? "1 " : "0 ");                                          // 51
            Data = Data + "&avgwdir=" + CompassPoint(AvgBearing);                                               // 52
            Data = Data + "&cloudbase=" + Convert.ToInt32(CloudBase).ToString();                                // 53
            Data = Data + "&cloudunit=" + (cumulus.CloudBaseInFeet ? "ft " : "m ");                             // 54
            Data = Data + "&apptemp=" + TempCstr(ApparentTemperature);                                          // 55
            Data = Data + "&sunhrs=" + SunshineHours.ToString(cumulus.SunFormat);                               // 56
            Data = Data + "&sunmax=" + Convert.ToInt32(CurrentSolarMax).ToString();                             // 57
            Data = Data + "&issunny=" + (IsSunny ? "1" : "0");                                                  // 58
        ...
        /// <summary>
		/// Convert pressure in inches of mercury 
		/// </summary>
		/// <param name="rain"></param>
		/// <returns></returns>
        ...
        /// <summary>
		/// Convert pressure in hectopascals
		/// </summary>
		/// <param name="pressure"></param>
		/// <returns></returns>
        private string PressHPAstr(double pressure)
        {
            var pressHPA = ConvertUserPressToMB(pressure);
            // milibar = hectopascal
            return pressHPA.ToString("F3");
        }
        ...
        /// <summary>
		/// Convert rain in user units to milimeters
		/// </summary>
		/// <param name="rain"></param>
		/// <returns></returns>
        private string RainMMstr(double rain)
        {
            var rainIN = ConvertUserRainToMM(rain);
        ...
        {
            double tempc = ConvertUserTempToC(temp);
		...
		public string GetThisMonthTempRecords()
		{
			...
			json +=
				monthyearjsonformat(AT_highdailytemprange, HighDailyTempRangeThisMonth, HighDailyTempRangeThisMonthTS, "&deg;" + cumulus.TempUnitText[1].ToString(),
					cumulus.TempFormat, "D") + ",";
			json += monthyearjsonformat(AT_lowdailytemprange, LowDailyTempRangeThisMonth, LowDailyTempRangeThisMonthTS, "&deg;" + cumulus.TempUnitText[1].ToString(),
				cumulus.TempFormat, "D");
		...
		public string GetThisYearTempRecords()
		{
			...
			json +=
				monthyearjsonformat(AT_highdailytemprange, HighDailyTempRangeThisYear, HighDailyTempRangeThisYearTS, "&deg;" + cumulus.TempUnitText[1].ToString(),
					cumulus.TempFormat, "D") + ",";
			json += monthyearjsonformat(AT_lowdailytemprange, LowDailyTempRangeThisYear, LowDailyTempRangeThisYearTS, "&deg;" + cumulus.TempUnitText[1].ToString(),
				cumulus.TempFormat, "D");
		...
		public string GetSoilMoisture()
		{
			...
			json += "[\"" + cumulus.SoilMoistureCaptions[1] + "\",\"" + SoilMoisture1.ToString("F0") + "\",\"cb" + "\"],";
			json += "[\"" + cumulus.SoilMoistureCaptions[2] + "\",\"" + SoilMoisture2.ToString("F0") + "\",\"cb" + "\"],";
			json += "[\"" + cumulus.SoilMoistureCaptions[3] + "\",\"" + SoilMoisture3.ToString("F0") + "\",\"cb" + "\"],";
			json += "[\"" + cumulus.SoilMoistureCaptions[4] + "\",\"" + SoilMoisture4.ToString("F0") + "\",\"cb" + "\"]";
	...
	public class GraphData
	{
		...
		public GraphData(DateTime ts, double rain, double raintoday, double rrate, double temp, double dp, double appt, double chill, double heat, double intemp, double press,
			double speed, double gust, int avgdir, int wdir, int hum, int inhum, double solar, double smax, double uv)
		{
			timestamp = ts;
			raincounter = rain;
			RainToday = raintoday;
