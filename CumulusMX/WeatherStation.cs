...
namespace CumulusMX
{
	internal abstract class WeatherStation
	{
		...
		private Object monthIniThreadLock = new Object();
		private Object yearIniThreadLock = new Object();
		private Object alltimeIniThreadLock = new Object();
		private Object monthlyalltimeIniThreadLock = new Object();
		...
		public void GetRainFallTotals()
		{
			...
			string Today = ModifiedNow.ToString("dd/MM/yy");
			cumulus.LogMessage("Today = " + Today);
			// get today"s date offset by rain season start for year check
			string OffsetToday = ModifiedNow.AddMonths(-(cumulus.RainSeasonStart - 1)).ToString("dd/MM/yy");
				...
				try
				{
					using (var sr = new StreamReader(cumulus.DayFile))
					{
						do
						{
							...
							{
								string datestr = st[0];
								DateTime offsetdate = ddmmyyStrToDate(datestr);
								string offsetdatestr = offsetdate.AddMonths(-(cumulus.RainSeasonStart - 1)).ToString("dd/MM/yy");
								// This year?
								if (offsetdatestr.Substring(6, 2) == OffsetToday.Substring(6, 2))
								{
									rainthisyear += Double.Parse(st[14]);
								}
								// This month?
								if ((datestr.Substring(3, 2) == Today.Substring(3, 2)) && (datestr.Substring(6, 2) == Today.Substring(6, 2)))
								{
...
//station = this,
		...
		public void SwitchToNormalRunning()
		{
			...
			cumulus.LogMessage("Switch to normal running");
			Console.WriteLine("Switch to normal running");
		...
		public void DoOutdoorHumidity(int humpar, DateTime timestamp)
		{
			...
			// apply offset and multiplier and round. This is different to C1, which truncates. I'm not sure why C1 does that
			OutdoorHumidity = (int) Math.Round((OutdoorHumidity*cumulus.HumMult) + cumulus.HumOffset);
